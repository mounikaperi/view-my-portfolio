...
function Projects(props) {
  ...
  const [filteredProjects, setFilteredProjects] = useState(categories[0]);
  ...
  return (
    <SectionWrapper id='projects' className='mx-4 md:mx-0 min-h-screen'>
      ...
      <div className='overflow-x-auto scroll-hide md:w-full max-w-screen-sm mx-auto mt-6 flex justify-between
      items-center gap-2 md:gap-3 bg-white dark:bg-grey-800 p-2 rounded-md'>
        { TECH_STACK_CATEGORIES.map((currentCategory, uniqueKey) => (
          <span key={uniqueKey} onClick={() => filterProjects(currentCategory)} 
      ...
      <div className='md:mx-6 lg:w-5/6 2xl:w-3/4 my-4 md:my-8 mx-auto grid md:grid-cols-2 xl:grid-cols-3
        gap-4 md:gap-10'>
        {
          filteredProjects.slice(0, viewAll ? filteredProjects.length: 6).map((currentProject, uniqueKey) => (
            <ProjectCard key={uniqueKey} {...currentProject} />
          ))
        }
      </div>
      {
