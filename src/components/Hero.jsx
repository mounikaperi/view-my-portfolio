...
import { DARK_THEME, LIGHT_THEME, NAME, SHORT_DESCRIPTION, TITLES } from '../utils/constants';
...
function Hero() {
  ...
  return (
    <section id='home' className={`${theme === 'dark' && "bg-grey-900"} relative min-h-screen w-full 
      mx-auto overflow-hidden`}>
        <div className="absolute -z-10 min-h-screen h-full w-full">
          <img src={imageUrl} layout="fill" style={{ objectFit: 'cover' }} loading='lazy' className='object-bottom'
            quality={100} alt={''} />
        </div>
        <div className="px-4 py-16 lg:py-36 flex flex-col-reverse lg:flex-row justify-around gap-4 lg:gap-0">
          <div className="lg:px-20 flex flex-col gap-4 md:gap-6 text-left lg:w-1/2 2xl:w-1/3 mx-8 md:mx-6 xl:mx-0">
            ...
            <h1 className="text-4xl md:text-5xl font-bold relative">I&apos;m {NAME}</h1>
            <div className="flex flex-row items-start md:items-center gap-1.5">
              <h3 className="text-lg md:text-2xl">I am into</h3>
              <TypewriterComponent options={{ strings: TITLES, autoStart: true, loop: true, deleteSpeed: 50,
                wrapperClassName: "text-violet-700 dark:text-violet-600 text-lg md:text-2xl font-medium",
                delay: 50, cursorClassName: "text-violet-700 dark:text-violet-600 text-lg md:text-2xl"
                }} />
            </div>
            <p className='text-sm md:text-base text-gray-600 dark:text-gray-300'>{SHORT_DESCRIPTION}</p>
          ...
          <div className="relative mx-auto lg:mx-0 mt-12 md:mt-16 lg:mt-0">
            ...
            <div className="absolute grid top-0 -right-12 lg:-right-4 w-14 h-14 bg-white dark:bg-grey-800 rounded-full place-items-center hover:shadow-lg transition-shadow">
              <img alt='tech-stack' className="h-8 w-8 md:h-10 md:w-10 object-cover" src={techStackImage2} width={100} height={100} />
            </div>
            <div className="absolute grid bottom-[4rem] md:bottom-24 -right-16 md:-right-20 lg:bottom-[8.5rem] lg:-right-12 w-12 h-12 md:w-16 md:h-16 bg-white dark:bg-grey-800 rounded-full place-items-center hover:shadow-lg transition-shadow">
              <img alt='tech-stack' className="h-6 w-6 md:h-10 md:w-10 object-cover" src={techStackImage3} width={100} height={100} />
            </div>
            <div className="absolute grid -bottom-10 -right-8 lg:-bottom-0 lg:right-6 w-14 md:w-16 h-14 md:h-16 bg-white dark:bg-grey-800 rounded-full place-items-center hover:shadow-lg transition-shadow">
              <img alt='tech-stack' className="h-10 w-10 object-cover" src={techStackImage4} width={100} height={100} />
            </div>
          </div>
        </div>
